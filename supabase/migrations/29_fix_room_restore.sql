-- 1. Redefine get_or_create_direct_room to restore deleted_at and accept screenshot protection preference
...
BEGIN
  ...
  IF existing_room_id IS NOT NULL THEN
    -- Restore the room participant entry for the current user if it was deleted
...
-- 2. Trigger function to automatically restore deleted rooms for all participants when a new message is sent
...
DROP TRIGGER IF EXISTS on_message_inserted_restore_rooms ON public.messages;
CREATE TRIGGER on_message_inserted_restore_rooms
  AFTER INSERT ON public.messages
  FOR EACH ROW
  EXECUTE FUNCTION public.handle_new_message_restore_rooms();
