-- 30: Fix history_cleared_at not being reset when room is restored
-- This causes messages to be invisible after re-creating a chat with the same contact
-- because the old history_cleared_at (possibly timezone-shifted) filters out new messages.

-- 1. Update get_or_create_direct_room to also reset history_cleared_at when restoring
...
BEGIN
  ...
  IF existing_room_id IS NOT NULL THEN
    -- Restore: clear both deleted_at AND history_cleared_at so old filter won't hide new messages
...
-- 2. Update trigger to also clear history_cleared_at when restoring on new message
